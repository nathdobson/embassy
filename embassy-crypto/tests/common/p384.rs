//! `embassy_crypto::p384`, cross-checked against the `p384` crate.
//!
//! With the `driver-p384-*` features off, every accelerated entry point
//! routes through the corresponding driver unitrait; this suite runs in both
//! configurations (the test binary provides a software driver impl when
//! needed). The SEC 2 base-point known answer anchors the suite to an
